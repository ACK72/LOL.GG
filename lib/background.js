...
let client = { reference: 0 }
let option = { updateFrequency: 1000 };
let contextTitle = {};
...
const setContextMenus = () => {
	...
	chrome.contextMenus.create({ title: 'Match History', contexts: ['browser_action'], onclick: () => {	if (client.summoner) chrome.tabs.create({ url: 'https://' + region[client.locale.region] + '.op.gg/summoner/userName=' + client.summoner.displayName }) } })
...
const onSession = () => {
	...
	if (client.mutex) {
		...
		switch (option.reference[client.queueType]) {
            case 'LOL.PS':
                if (client.cid) rurl = 'https://lol.ps/champ/' + client.cid
